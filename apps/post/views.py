...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, pk=None):
        ...
        if not is_owner(request, post):
            return Response({'message': 'You are not authorized to update this post'}, status=status.HTTP_401_UNAUTHORIZED)
        else:
            if 'image' not in request.data or request.data['image'] == '':
                ...
                post_serializer = PostCreateSerializer(post, data=data)
                if post_serializer.is_valid():
                    post_serializer.save()
                    return Response({'message': 'Post updated successfully', 'data': post_serializer.data}, 
